...
export function generateMatrix(key) {
  ...
  const matrix = [];
  for (let i = 0; i < 5; i++) {
    const row = uniqueKey.slice(i * 5, (i + 1) * 5).split("");
    // Replace 'I' with 'I/J' in display
    const displayRow = row.map((char) => (char === "I" ? "I/J" : char));
...
export function findPosition(matrix, letter) {
  // Normalize I/J handling
  const searchLetter = letter === "J" ? "I/J" : letter;

  for (let row = 0; row < 5; row++) {
    for (let col = 0; col < 5; col++) {
      if (
        matrix[row][col] === searchLetter ||
        (matrix[row][col] === "I/J" && (letter === "I" || letter === "J"))
      ) {
        return { row, col };
      }
    }
...
function encryptDigraph(matrix, digraph) {
  ...
  let result = "";
  // Convert I/J back to I in the result
  let char1 = matrix[newPos1.row][newPos1.col];
  let char2 = matrix[newPos2.row][newPos2.col];
  result += char1 === "I/J" ? "I" : char1;
  result += char2 === "I/J" ? "I" : char2;
  return result;
}
